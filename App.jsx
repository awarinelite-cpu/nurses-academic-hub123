...
// ─── FIRESTORE HELPERS ───────────────────────────────────────────────
// In-memory cache so synchronous ls() still works for reads
// that have already been fetched, while async writes go to Firestore.
const _cache = {};
const _listeners = {};

/** Notify local React listeners */
const _notify = (key, val) => {
  _cache[key] = val;
  (_listeners[key] || []).forEach(fn => fn(val));
};

/** Synchronous read from cache (falls back to default) */
const ls = (k, d) => (_cache[k] !== undefined ? _cache[k] : d);

/** Write to Firestore AND update local cache */
const lsSet = async (k, v) => {
  _notify(k, v);
  try {
    await setDoc(doc(_db, "store", k), { value: v });
  } catch (e) {
    console.error("Firestore write failed:", e);
  }
};

/** React hook: subscribe to a key, hydrate from Firestore on first mount */
const useLs = (key, def) => {
  ...
  React.useEffect(() => {
    ...
    // Hydrate from Firestore if not already cached
    if (_cache[key] === undefined) {
      getDoc(doc(_db, "store", key)).then(snap => {
...
/** Prefetch a list of keys into cache (call once on app start) */
const prefetchKeys = async (keys) => {
  await Promise.all(keys.map(async k => {
    if (_cache[k] !== undefined) return;
    const snap = await getDoc(doc(_db, "store", k));
    if (snap.exists()) _notify(k, snap.data().value);
  }));
};
...
// ─── INIT STORAGE ───────────────────────────────────────────────────
// Admin account is now managed via Firebase Auth + Firestore user profile.
// The special admin UID is stored in Firestore at store/nv-admin-uid.
// Legacy username/password users are stored in store/nv-users as before.
const ADMIN_ACCOUNT = {username:"admin",password:"admin123",role:"admin",class:"",joined:"System"};

const ALL_KEYS = [
  "nv-classes","nv-drugs","nv-labs","nv-pq","nv-decks","nv-dict",
  "nv-skillsdb","nv-announcements","nv-folders","nv-handouts","nv-exams",
  "nv-users","nv-timetable","nv-lect-notes","nv-lect-results",
];

const DEFAULTS_MAP = {
  "nv-classes": DEFAULT_CLASSES,
  "nv-drugs": DEFAULT_DRUGS,
  "nv-labs": DEFAULT_LABS,
  "nv-pq": DEFAULT_PQ,
  "nv-decks": DEFAULT_DECKS,
  "nv-dict": DEFAULT_DICT,
  "nv-skillsdb": DEFAULT_SKILLS,
  "nv-announcements": DEFAULT_ANNOUNCEMENTS,
  "nv-folders": [],
  "nv-handouts": [],
  "nv-exams": [],
  "nv-timetable": [],
  "nv-lect-notes": [],
  "nv-lect-results": [],
  "nv-users": [ADMIN_ACCOUNT],
};

/**
 * Seeds Firestore with default data for any key that doesn't exist yet.
 * Runs once on app load. Safe to call multiple times (no-op if data exists).
 */
const initData = async () => {
  try {
    await Promise.all(ALL_KEYS.map(async key => {
      const snap = await getDoc(doc(_db, "store", key));
      if (!snap.exists()) {
        let defaultVal = DEFAULTS_MAP[key] ?? [];
        // Ensure admin account exists in nv-users
        if (key === "nv-users") {
          const hasAdmin = defaultVal.some(u => u.username === "admin" && u.role === "admin");
          if (!hasAdmin) defaultVal = [ADMIN_ACCOUNT, ...defaultVal];
        }
        await setDoc(doc(_db, "store", key), { value: defaultVal });
        _notify(key, defaultVal);
      } else {
        _notify(key, snap.data().value);
      }
    }));
  } catch (e) {
    ...
    <div>
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <button className="btn btn-danger" onClick={()=>{{Object.keys(_cache).forEach(k=>delete _cache[k]);initData();toast("Data reset to defaults","warn");}}}>🔄 Reset to Defaults</button>
        </div>
      </div>
      <div className="card">
...
function AdminUsers({ toast }) {
  ...
  // Admin can create accounts on behalf of users.
  // Password is set via Firebase Auth — never stored in Firestore.
  const save = async () => {
    if (!form.username) return toast("Username required","error");
    setSaving(true);
    try {
      if (edit) {
        // Update role/class only — cannot change password here (user must reset it)
        const u = users.map(x=>x.username===edit?{...x,role:form.role,class:form.class}:x);
        setUsers(u); await lsSet("nv-users",u);
        // Also update their userProfile doc if it exists
        const profileSnap = await getDocs(collection(_db, "userProfiles"));
        const profileDoc = profileSnap.docs.find(d=>d.data().username===edit);
        if (profileDoc) await updateDoc(doc(_db,"userProfiles",profileDoc.id),{role:form.role,class:form.class});
        ...
        if (users.find(u=>u.username===form.username)) return toast("Username already exists","error");
        const cred = await createUserWithEmailAndPassword(_auth, toFbEmail(form.username), form.password);
        const profile = { username:form.username, role:form.role, class:form.class, joined:new Date().toLocaleDateString(), uid:cred.user.uid };
        await setDoc(doc(_db,"userProfiles",cred.user.uid), profile);
        const u = [...users, profile];
        setUsers(u); await lsSet("nv-users",u);
        toast("User created with Firebase Auth ✅","success");
      }
    } catch(err) {
      toast(err.code==="auth/email-already-in-use"?"Username already exists":err.message,"error");
    } finally {
      setSaving(false); setEdit(null); setShowAdd(false); setForm({username:"",password:"",role:"student",class:""});
    }
  };

  const del = async (username) => {
    if (username==="admin") return toast("Cannot delete admin","error");
    const u = users.filter(x=>x.username!==username); setUsers(u); await lsSet("nv-users",u);
    toast("User removed from roster (Firebase Auth account still exists — delete manually in Firebase Console if needed)","warn");
  };
  ...
  return (
    <div>
      ...
      <div style={{background:"rgba(74,222,128,.07)",border:"1px solid rgba(74,222,128,.2)",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,color:"var(--success)",fontFamily:"'DM Mono',monospace"}}>
        🔐 Passwords are secured by Firebase Authentication — they are never stored in the database.
      </div>
            ...
            {filtered.map(u=>(
              <tr key={u.username}>
                ...
                <td><div className="tbl-actions">
                  <button className="btn btn-sm" onClick={()=>{setEdit(u.username);setForm({username:u.username,password:"",role:u.role||"student",class:u.class||""});setShowAdd(true);}}>✏️ Edit</button>
      ...
      {showAdd&&(
        <div className="modal-overlay" onClick={()=>setShowAdd(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <input className="inp" value={form.username} onChange={e=>setForm({...form,username:e.target.value})} disabled={!!edit} placeholder="Letters, numbers, underscores only" />
            {!edit && <>
              <label className="lbl">Password (min 6 chars)</label>
              <input className="inp" type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="Temporary password for this user" />
            </>}
            {edit && <div style={{fontSize:11,color:"var(--text3)",fontFamily:"'DM Mono',monospace",marginBottom:13,background:"var(--bg4)",borderRadius:8,padding:"8px 12px"}}>🔐 Password is managed by Firebase Auth. To reset a password, delete and re-create the account.</div>}
            ...
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-purple" style={{flex:1}} onClick={save} disabled={saving}>{saving?"Saving…":"Save"}</button>
  ...
  const save = async () => {
    ...
    try {
      ...
      if (edit) {
        ...
        const profileSnap = await getDocs(collection(_db,"userProfiles"));
        const profileDoc = profileSnap.docs.find(d=>d.data().username===edit);
        if (profileDoc) await updateDoc(doc(_db,"userProfiles",profileDoc.id),{title:form.title,department:form.department,specialization:form.specialization,bio:form.bio});
        toast("Lecturer updated","success");
      } else {
        if (form.password.length < 6) return toast("Password must be at least 6 characters","error");
        const cred = await createUserWithEmailAndPassword(_auth, toFbEmail(form.username), form.password);
        const profile = {...form, role:"lecturer", joined:new Date().toLocaleDateString(), uid:cred.user.uid};
        await setDoc(doc(_db,"userProfiles",cred.user.uid), profile);
        u = [...users, profile];
        toast("Lecturer account created with Firebase Auth ✅","success");
      }
        ...
        <div className="modal-overlay" onClick={()=>setShowModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="form-row">
              ...
              <div><label className="lbl">Password</label><input className="inp" type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="set a password" /></div>
            </div>
            <div className="form-row">
...
function ClassGroupChat({ currentUser, classId, className: className_ }) {
  const colPath = `chats/${classId}/messages`;
  ...
  useEffect(() => {
    const q = query(collection(_db, colPath), orderBy("createdAt", "asc"));
    const unsub = onSnapshot(q, snap => {
      const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      setMsgs(data.length ? data : [{id:"sys",from:"System",text:`Welcome to the ${className_} group chat! 👋`,role:"system",time:""}]);
  ...
  const send = async () => {
    ...
    await addDoc(collection(_db, colPath), {
      from: currentUser,
      text: input.trim(),
  ...
  const sendDM = async () => {
    ...
    const dmCol = `dms/${[currentUser,dmTarget].sort().join("-")}/messages`;
    await addDoc(collection(_db, dmCol), {
      from: currentUser,
      to: dmTarget,
      text: dmInput.trim(),
      time: new Date().toLocaleTimeString(),
...
function LecturerGroupChat({ toast, currentUser }) {
  const colPath = "chats/lecturers/messages";
  const [msgs, setMsgs] = useState([]);
  const [input, setInput] = useState("");
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      const data = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      setMsgs(data.length ? data : [{id:"sys",from:"System",text:"Welcome to the Lecturer Group Chat! 🎓 Share updates, coordinate schedules, and collaborate here.",role:"system"}]);
  ...
  const del = async (id) => {
    await deleteDoc(doc(_db, colPath, id));
  };
...
// ════════════════════════════════════════════════════════════════════
// ─── AUTH HELPERS ────────────────────────────────────────────────────
// Users log in with a username but Firebase Auth needs an email.
// We use username@nursevault.internal as a synthetic email — users never see it.
const toFbEmail = (username) => `${username.trim().toLowerCase()}@nursevault.internal`;

export default function App() {
  const [appReady, setAppReady] = useState(false);

  // 1. Seed Firestore with defaults
  // 2. Check if user is already signed in (persists across page refresh)
  ...
  // On mount: seed data, then check for existing Firebase Auth session
  useEffect(() => {
    let unsub = () => {};
    initData()
      .then(() => {
        unsub = onAuthStateChanged(_auth, async (firebaseUser) => {
          if (firebaseUser) {
            // User was already signed in — restore their session
      ...
      .catch(() => {
        // If initData fails, still unblock the loading screen
        setAppReady(true);
      });
    return () => unsub();
  }, []);

  // Fetch user profile from Firestore by Firebase UID
  ...
  const login = async () => {
    ...
    setAuthLoading(true);
    try {
      const cred = await signInWithEmailAndPassword(_auth, toFbEmail(username), password);
      const profile = await _getUserProfile(cred.user.uid);
      if (!profile) { await signOut(_auth); return toast("Account not found. Try registering.", "error"); }
      setCurrentUser(profile.username);
      setIsAdmin(profile.role === "admin");
      setIsLecturer(profile.role === "lecturer");
      setPage("app");
  ...
  const register = async () => {
    ...
    if (regUser.trim().length < 3) return toast("Username must be at least 3 characters", "error");
    if (regPw.length < 6) return toast("Password must be at least 6 characters", "error");
    if (!/^[a-zA-Z0-9_]+$/.test(regUser.trim())) return toast("Username can only contain letters, numbers, and underscores", "error");
    setAuthLoading(true);
    try {
      // Create Firebase Auth account (password is hashed by Firebase — never stored as plain text)
      const cred = await createUserWithEmailAndPassword(_auth, toFbEmail(regUser), regPw);

      // Save profile to Firestore (no password field — Firebase Auth handles it)
      const profile = {
      ...
      // Also update the nv-users list (so admin panel still works)
      const users = ls("nv-users", []);
      await lsSet("nv-users", [...users, { ...profile }]); // no password field

      setCurrentUser(profile.username);
      setIsAdmin(false);
      setIsLecturer(false);
      setPage("app");
      toast(`Welcome to NurseVault, ${profile.username}! 🎉`, "success");
    } catch (err) {
  ...
  // ── ADMIN ACCESS (gear button) ────────────────────────────────────────
  const verifyAdminAccess = async () => {
    if (!adminAuthUser || !adminAuthPw) return toast("Enter credentials", "error");
    setAuthLoading(true);
    try {
      const cred = await signInWithEmailAndPassword(_auth, toFbEmail(adminAuthUser), adminAuthPw);
      const profile = await _getUserProfile(cred.user.uid);
      if (!profile || profile.role !== "admin") {
        await signOut(_auth);
        return toast("Access denied — not an admin account", "error");
      }
      setCurrentUser(profile.username);
      setIsAdmin(true);
      setIsLecturer(false);
  ...
  if (!appReady) return (
    <>
      <style>{CSS}</style>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"var(--bg)",gap:18}}>
        <div style={{width:52,height:52,borderRadius:14,background:"linear-gradient(135deg,var(--accent),var(--accent2))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26}}>🏥</div>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,color:"var(--accent)"}}>NurseVault</div>
        <div style={{width:40,height:40,border:"3px solid var(--border)",borderTopColor:"var(--accent)",borderRadius:"50%",animation:"spin 0.8s linear infinite"}} />
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:11,color:"var(--text3)"}}>Connecting to backend…</div>
      </div>
    </>
  );
          ...
          <div className="auth-card">
            ...
            {authTab==="signin" ? (
              ...
              <>
                ...
                <label className="lbl">Password</label>
                <input className="inp" type="password" placeholder="Min 6 characters" value={regPw} onChange={e=>setRegPw(e.target.value)} />
